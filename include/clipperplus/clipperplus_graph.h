...
public:
    ...
    int size() const;
    Weight degree(Node v) const;
    std::vector<Weight> degrees() const;

    const std::vector<Neighbor> &neighbors(Node v) const;
    ...
    Weight max_core_number() const;
    const std::vector<Weight> &get_core_numbers() const;
...
private:
    ...
    mutable std::vector<Weight> kcore;
};
