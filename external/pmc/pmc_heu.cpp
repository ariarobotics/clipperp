...
inline void pmc_heu::branch(vector<Vertex>& P, int sz,
        int& mc, vector<long>& C, vector<short>& ind) {

    if (!P.empty()) {

        long u = P.back().get_id();
        P.pop_back();

        for (long j = (*V)[u]; j < (*V)[u + 1]; j++)  ind[(*E)[j]] = 1;

        vector <Vertex> R;
        R.reserve(P.size());
        for (int i = 0; i < P.size(); i++){
            if (ind[P[i].get_id()] && (*K)[P[i].get_id()] > mc){   
                R.push_back(P[i]);
            }
        }
        for (long j = (*V)[u]; j < (*V)[u + 1]; j++)  ind[(*E)[j]] = 0;
...
long pmc_heu::compute_heuristic(long v) {
...
int pmc_heu::search_cores(const pmc_graph& G, vector<long>& C_max, int lb) {
...
int pmc_heu::search(pmc_graph& G, vector<long>& C_max) {
    return search_bounds(G, C_max);
}


inline void pmc_heu::print_info(const vector<long>& C_max) const {
