...
void pmc_graph::basic_stats (double sec) const {
...
void pmc_graph::read_edges(const string& filename) {
    ...
    for (const auto& vertex : vert_list) {
        vector<int> neighborhood = vertex.second;
        edges.insert(edges.end(), neighborhood.begin(), neighborhood.end());
        vertices.push_back(edges.size());
    }
...
pmc_graph::pmc_graph(long nedges, const int *ei, const int *ej, int offset) {
    map< int, vector<int> > vert_list;
...
pmc_graph::pmc_graph(map<int,vector<int>> const & v_map) {
  vertices.push_back(edges.size());
...
void pmc_graph::read_mtx(const string& filename) {
    ...
    for (char* p=data_type; *p!='\0'; *p=static_cast<char>(tolower(*p)),p++);
...
void pmc_graph::read_metis(const string& filename) const{ return; };

void pmc_graph::create_adj() {

    unsigned long size = num_vertices();
    adj.resize(size);
...
void pmc_graph::sum_vertex_degrees() {
    unsigned long n = vertices.size() - 1;
...
void pmc_graph::update_degrees(int* &pruned, const int& mc) {
...
double pmc_graph::density() const {

    auto numerator = static_cast<double>(num_edges());
    auto num_vertices_to_double = static_cast<double>(num_vertices());
    auto denominator = (num_vertices_to_double * (num_vertices_to_double - 1.0) / 2.0);
...
void pmc_graph::update_kcores(int* &pruned) {
    ...
    for (long d=md; d > 1; d--)  bin[d] = bin[d-1];
    bin[0] = 1;
...
string pmc_graph::get_file_extension(const string& filename) const{
...
void pmc_graph::bound_stats(int alg) const {
    cout << "graph: " << fn <<endl;
    cout << "alg: " << alg <<endl;
    cout << "-------------------------------" <<endl;
...
void pmc_graph::compute_ordering(vector<int>& bound, vector<long>& order) const {
    long n;
    long d;
    long start;
    long num;
...
bool pmc_graph::clique_test(pmc_graph& G, vector<int> C) const{
    int u = 0;
    vector<short> ind(G.num_vertices(),0);
    for (auto vertex : C) ind[vertex] = 1;

    // ensure each vertex in C has |C|-1 edges between each other
    for (auto vertex : C) {
        u = vertex;
        int sz = 0;
        for (long j = G.vertices[u]; j < G.vertices[u+1]; j++)
            if (ind[G.edges[j]])  sz++;
