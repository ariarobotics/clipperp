...
std::vector<long> findIndicesOfkLargest(const Eigen::VectorXd& x, int k)
{
...
std::vector<long> findIndicesWhereAboveThreshold(const Eigen::VectorXd& x,
                                                double thr)
{
...
std::tuple<size_t,size_t> k2ij(size_t k, size_t n)
{
  ...
  const auto o = static_cast<size_t>(std::floor( (std::sqrt(1 + 8*l) - 1) / 2. ));
