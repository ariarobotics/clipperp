...
void pybind_dsd(py::module& m)
{
  ...
  m.def("solve", py::overload_cast<const Eigen::MatrixXd&,
                        const std::vector<long>&>(clipper::dsd::solve),
    "A"_a.noconvert(), "S"_a=std::vector<long>{},
    "Find densest edge-weighted subgraph of weighted adj mat A.");
}

// ----------------------------------------------------------------------------

void pybind_clipperplus(py::module& m)
{
...
PYBIND11_MODULE(clipperpy, m)
{
  ...
  // CLIPPER+ 
  py::module m_clipperplus = m.def_submodule("clipperplus");
  pybind_clipperplus(m_clipperplus);
}
